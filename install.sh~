...
install_dependencies() {
    ...
    apt-get install -y rpm-build curl jq python3-module-bcc sqlite3
}

get_latest_release_url() {
    info "Поиск последнего релиза на GitHub..."
    local latest_url
    latest_url=$(curl -s "$API_URL" | jq -r '.tarball_url')

    if [ -z "$latest_url" ] || [ "$latest_url" == "null" ]; then
        error "Не удалось найти URL последнего релиза. Проверьте репозиторий или ваше интернет-соединение."
    fi
    echo "$latest_url"
}
